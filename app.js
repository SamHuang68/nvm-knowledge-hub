...
const staticTextPairs = [
  ...
  ["RP2350 是一個現實提醒：fault、permission、wrapper、detector 與 bit-cell 都可能成為路徑。產品設計必須限制每條路徑成功後的可得物。", "RP2350 is a practical reminder that fault, permission, wrapper, detector and bit-cell behavior can all become attack paths. Product design must limit what each successful path can recover."],
  ...
  ["讓 bit-cell readout 不等於 secret disclosure。", "Make a bit-cell readout different from secret disclosure."],
  ["controller、lifecycle 與 evidence 同時納入交付。", "Deliver the controller, lifecycle and evidence as one outcome."],
  ["不是漏洞事件回顧，而是 threat model 的產品設計輸入。", "Not a vulnerability retrospective, but a product-design input to the threat model."],
