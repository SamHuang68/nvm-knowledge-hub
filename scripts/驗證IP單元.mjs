...
const requiredIds=['neobit','neofuse','kilopass-xpm','sidense-1t-fuse','cfx-otp','attopsemi-ifuse','floadia-za','neoee','neomtp','ymc-mtp','impinj-aeon','actt-cmt','nscore-twinbit','floadia-zt','floadia-g1','floadia-g2','sst-superflash','numem-mram','gf-emram','weebit-reram','crossbar-reram'];
note(units.length===requiredIds.length&&requiredIds.every(id=>units.some(unit=>unit.id===id))&&new Set(units.map(unit=>unit.id)).size===units.length,'IP 目錄包含既有單元及 Actt、NSCore、Floadia、創飛芯、Attopsemi、SST 技術');
