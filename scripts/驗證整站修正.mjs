...
const scripts=['驗證檢查門禁.mjs','驗證模擬備援.mjs','驗證搜尋與狀態修正.mjs','驗證全景延後載入.mjs','驗證離線快取.mjs','../tools/whitepaper-studio/scripts/驗證白皮書瀏覽器.mjs'];
try {
  for(const file of scripts) {
    console.log(`開始驗證：${file}`);
    const env={...process.env,NVM_QA_BASE:server.base,NVM_QA_CHANNEL:process.env.NVM_QA_BROWSER||'msedge'};
    if(file==='驗證離線快取.mjs') delete env.NVM_QA_BASE;
