...
import {collectIPCurriculum, renderIPDirectory, renderIPNavigation, renderIPPanels, ipCurriculumMarkdown} from './NVMIP導讀介面.mjs';
...
const ipCurriculum = collectIPCurriculum(ipIndex, language, id => ['neobit','neofuse','neoee','neomtp'].includes(id) ? ememoryIPStudy : id === 'ymc-mtp' ? ymcIPStudy : emergingIPStudy);
const originalSources = [...intro.sources, ...charge.sources, ...emerging.sources, ...comparison.sources, ...foundry.sources, ...ipCurriculum.sources];
...
validateRefs({ topics, comparison, foundry, ipCurriculum }, '內容');
...
const serialized = JSON.stringify({ intro, topics, sources, comparison, foundry, engineering, ipCurriculum });
...
${implementationTable(topic)}
  ...
  ${topic.operations.map((operation,i) => `<div id="op-${esc(topic.id)}-${esc(operation.id)}" data-operation-detail="${esc(operation.id)}"><h4>${esc(operation.title)}</h4>${renderOperationStudy(engineeringOperations.get(topic.id+'-'+operation.id),language)}<details class="nvm-operation-text"><summary>${isEnglish?'Read the Full Operation Explanation':'閱讀完整操作解釋'}</summary><dl class="nvm-state-sequence"><div><dt>操作前</dt><dd>${esc(operation.before)}</dd></div><div><dt>施加的刺激</dt><dd>${esc(operation.stimulus)}</dd></div><div><dt>操作後</dt><dd>${esc(operation.after)}</dd></div></dl>${paras(operation.explanation)}</details></div>`).join('')}</div></section>
...
const panorama = `<article id="panorama" class="nvm-panel" data-nvm-panel>${panelHeader(isEnglish?'Embedded NVM IP · Cell Operation':'嵌入式 NVM IP · 單元操作',isEnglish?'IP Cells: Where State Lives and How It Changes':'從 IP 單元，看懂狀態如何寫入與改變',intro.intro)}
${renderIPDirectory(ipCurriculum,language)}
<details class="nvm-physical-background" id="nvm-physics-overview"><summary>${isEnglish?'Device-Physics Background, Standalone Components and Further Reading':'物理背景、獨立式元件與延伸閱讀'}</summary>
...
<html lang="${isEnglish ? 'en' : 'zh-Hant'}" data-language="${language}" data-content-language="${language}" data-language-en="NVM技術全景.html" data-language-zh="NVM技術全景中文.html"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(isEnglish ? 'Nine named OTP, MTP, MRAM and ReRAM IP cell studies with editable structure and operation diagrams, public sources and sixteen device-physics references.' : '九款具名 OTP、MTP、MRAM、ReRAM IP 單元，提供可編輯結構與逐格操作圖、公開來源及十六個物理背景專題。')}"><meta name="theme-color" content="#0a1118"><title>${esc(intro.title)} · NVM Knowledge Hub</title><link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="NVM技術全景.css?v=20260910-bilingual"><link rel="stylesheet" href="NVM專業元件圖.css?v=20260910"><link rel="stylesheet" href="NVM工程圖解.css?v=20260910"><link rel="stylesheet" href="NVMIP單元.css?v=20260910-ip"><script src="site-language.js?v=20260910-bilingual"></script></head>
...
<header class="nvm-header"><a class="brand" href="index.html" aria-label="NVM 知識中心首頁"><strong>NVM</strong><span>知識中心<br>物理與技術全景</span></a><nav aria-label="知識中心導覽"><a href="index.html">知識中心首頁</a><a href="#panorama">${isEnglish?'IP Cell Studies':'IP 單元主線'}</a><a href="#foundry">晶圓代工路線圖</a><a href="#sources">來源與下載</a><button type="button" class="language-toggle" aria-label="${isEnglish ? 'Switch to Traditional Chinese' : '切換為英文'}"><span lang="en">EN</span><i aria-hidden="true">/</i><span lang="zh-Hant">中文</span></button></nav></header>
<section class="nvm-hero"><div><p class="nvm-kicker">${isEnglish ? 'EMBEDDED NVM IP / CELL OPERATING PRINCIPLES' : '嵌入式 NVM IP／單元操作原理'}</p><h1>${isEnglish ? 'Inside the IP Cell<br><em>Follow How State Changes</em>' : '從 IP 的單元<br><em>看懂寫入與保存</em>'}</h1><p>${isEnglish ? 'Named OTP, MTP, MRAM and ReRAM IP cells. Follow the storage region, carrier or spin path, reverse operation and readout, one state at a time.' : '以具名 OTP、MTP、MRAM、ReRAM IP 為主線，逐格追蹤儲存區、載子或自旋路徑、反向操作與讀取。'}</p></div><div class="nvm-hero-figure"><div class="nvm-figure-label">${isEnglish ? 'IP CELL / ACCESS / SENSE' : 'IP 單元／選擇／感測'}</div>${bitcellFigure('stt','STT-MRAM',language,true)}<div class="nvm-figure-caption">${isEnglish ? 'MTJ Principles · Continue to the Named IP Cell Studies' : '磁性接面原理 · 接續閱讀具名 IP 單元'}</div></div></section>
<nav class="nvm-breadcrumb" aria-label="麵包屑導覽"><a href="index.html">NVM 知識中心</a> ／ <span>NVM 技術全景</span></nav>
<div class="nvm-layout"><aside class="nvm-sidebar"><button type="button" class="nvm-contents-button" id="nvm-contents-toggle" aria-expanded="false" aria-controls="nvm-contents">選擇閱讀主題 <span aria-hidden="true">＋</span></button><nav id="nvm-contents" aria-label="專題目錄"><a href="#panorama">${isEnglish?'IP Cells and Operating Paths':'IP 單元與操作路徑'}</a>${renderIPNavigation(ipCurriculum,language)}<details class="nvm-background-nav"><summary>${isEnglish?'Physics and Reference Topics':'物理背景與參考專題'}</summary><a href="#nvm-physics-overview">${isEnglish?'Physical Families and Reference Table':'物理家族與背景總表'}</a><a href="#comparison">歷史表與現行比較</a><a href="#foundry">GF／TSMC 年度路線圖</a>${intro.families.map(family=>`<h3>${esc(family.title)}</h3>${family.topics.map(id=>`<a class="nvm-topic-link" href="#topic-${id}">${esc(topics.find(topic=>topic.id===id).title)}</a>`).join('')}`).join('')}</details><h3>陣列、系統與證據</h3>${comparison.systems.map(system=>`<a href="#system-${esc(system.id)}">${esc(system.title)}</a>`).join('')}<a href="#patents">${patents.length} 件代表專利</a><a href="#glossary">共同詞彙</a><a href="#sources">來源與資料下載</a></nav></aside>
<main class="nvm-main" id="main-content"><noscript><p class="nvm-nojs">此頁已包含全部專題文字。啟用 JavaScript 可使用分頁閱讀、篩選與操作切換。</p></noscript>${panorama}${renderIPPanels(ipCurriculum,language,cite)}${comparisonPanel()}${foundryPanel()}${topics.map(topicPanel).join('')}${comparison.systems.map(systemPanel).join('')}${patentPanel}${glossaryPanel}${sourcePanel}</main></div>
<footer class="nvm-footer"><p>NVM 知識中心 · 公開來源研究 · ${intro.revision}</p><a href="index.html">返回知識中心</a> · <a href="#sources">查看來源與共用資料</a></footer><script type="module" src="NVM技術全景.js?v=20260910-ip-cells"></script><script type="module" src="NVM工程圖解.js?v=20260910"></script></body></html>\n`;
// 全頁標題使用標籤語法，不保留句尾標點。
html = html.replace(/<h([1-6])\b[^>]*>[\s\S]*?<\/h\1>/giu, heading => heading.replace(/[。.!?！？](?=(?:[”’"'）)}\]】》]+)?\s*(?:<br\b[^>]*>|<\/(?:span|em|h[1-6])>))/giu,''));
const packageData = { schemaVersion: '1.2', revision: intro.revision, language: isEnglish ? 'en' : 'zh-Hant', classification: 'Public', title: intro.title, intro, topics, comparison, foundry, sources, engineering, ipCurriculum, provenance: { sourceFiles: ['NVM全景導論.json','NVM電荷專題.json','NVM新興專題.json','NVM比較與系統.json','NVM晶圓代工路線圖.json','NVMIP單元導論.json'].map(name => name.replace('.json', `${dataSuffix}.json`)), engineeringSources: ['NVM專利附圖.json', '../scripts/NVM電荷操作圖.mjs', '../scripts/NVM新興操作圖.mjs', '../scripts/NVM專利圖解.mjs','../scripts/NVM力旺IP圖解.mjs','../scripts/NVM新興IP圖解.mjs','../scripts/NVMYMCIP圖解.mjs'], reviewScope: isEnglish ? 'Public-source research and instructional review; shared source for future presentations' : '公開來源與教學內容查核；網頁為後續簡報的取材來源' } };
