...
import { collectEngineeringStudies, renderOperationStudy, renderPatentGuide, renderPatentTeaser } from './NVM工程圖解介面.mjs';
import {collectIPCurriculum, renderIPDirectory, renderIPNavigation, renderIPPanels, renderIPLineage, ipCurriculumMarkdown, ipLineageMarkdown} from './NVMIP導讀介面.mjs';
import {getIPStudy as ememoryIPStudy} from './NVM力旺IP圖解.mjs';
import {getIPStudy as emergingIPStudy} from './NVM新興IP圖解.mjs';
...
const research = read('NVM產研比較.json');
...
${referenceOnly ? `<aside class="nvm-reference-intro"><p>${isEnglish ? 'A standalone EEPROM is a packaged component with a controller and external interface. Use it here to understand the component boundary; the main learning path follows embedded IP cells and their operating mechanisms.' : '獨立式 EEPROM 是帶控制器與外部介面的封裝元件。此處只用它理解元件邊界；主要閱讀路徑放在嵌入式 IP 單元與各自操作機制。'}</p><a href="#ip-neoee">${isEnglish ? 'Continue with NeoEE FN/FN MTP' : '接著看 NeoEE FN／FN MTP'}</a> · <a href="#ip-neomtp">${isEnglish ? 'Compare NeoMTP CHI/FN' : '比較 NeoMTP CHI／FN'}</a></aside><details class="nvm-reference-body" id="eeprom-reference" open><summary>${isEnglish ? 'Optional Reference: Standalone EEPROM Details' : '背景補充：獨立式 EEPROM 詳細資料'}</summary>` : ''}
...
function historicalComparison(historical) {
  return `<section id="comparison-history" class="nvm-comparison-history nvm-history-editorial"><p class="nvm-kicker">${isEnglish?'HISTORICAL REFERENCE':'歷史資料對照'}</p><h3>從 2016 文獻到 2021 課程表</h3>${paras(historical.intro)}
  <nav class="nvm-history-index" aria-label="${isEnglish?'Historical Reference Sections':'歷史資料章節'}"><a href="#comparison-history-table"><span>01</span><strong>${isEnglish?'Explore the Historical Table':'查閱歷史比較表'}</strong><small>${historical.columns.length} ${isEnglish?'technologies':'類技術'} · ${historical.rows.length} ${isEnglish?'metrics':'個指標'}</small></a><a href="#comparison-corrections"><span>02</span><strong>${isEnglish?'Read the Evidence Corrections':'閱讀差異校正'}</strong><small>${historical.corrections.length} ${isEnglish?'corrections with sources':'則校正與來源'}</small></a></nav>
  <details id="comparison-history-table" class="nvm-disclosure nvm-history-disclosure"><summary>${isEnglish?'Historical Table — Course Reference Only':'歷史比較表：僅供課程對照'}</summary><div class="nvm-history-body"><div class="nvm-reference-controls nvm-history-controls"><label>${isEnglish?'Comparison Metric':'比較指標'}<select id="nvm-history-metric"><option value="">${isEnglish?'All Metrics':'全部指標'}</option>${historical.rows.map((row,i)=>`<option value="${i}">${esc(row.metric)}</option>`).join('')}</select></label><button type="button" id="nvm-history-reset">${isEnglish?'Show All Metrics':'顯示全部指標'}</button></div><p id="nvm-history-count" class="nvm-reference-count" role="status">${isEnglish?'Historical course values; not current product specifications.':'以下為歷史課程數值，不是現行產品規格。'}</p>
  <details class="nvm-history-notes"><summary>${isEnglish?'Measurement Conditions and Limits':'測量條件與限制'}</summary><div>${historical.rows.map((row,i)=>row.note?`<p data-history-metric="${i}"><strong>${esc(row.metric)}</strong>${esc(row.note)}</p>`:'').join('')}</div></details>
  <div class="nvm-history-matrix"><table class="nvm-table nvm-history-table"><caption>課程截圖的歷史比較，非現行產品規格</caption><thead><tr><th scope="col">比較項目</th>${historical.columns.map(column=>`<th scope="col">${esc(column)}</th>`).join('')}</tr></thead><tbody>${historical.rows.map((row,i)=>`<tr data-history-metric="${i}"><th scope="row">${esc(row.metric)}</th>${row.values.map(value=>`<td>${esc(value)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>
  <div class="nvm-history-cards">${historical.columns.map((column,i)=>`<article><h4>${esc(column)}</h4><dl>${historical.rows.map((row,j)=>`<div data-history-metric="${j}"><dt>${esc(row.metric)}</dt><dd>${esc(row.values[i])}</dd></div>`).join('')}</dl></article>`).join('')}</div></div></details>
  <section id="comparison-corrections" class="nvm-corrections"><h3>${isEnglish?'Read the Evidence Corrections':'閱讀差異校正'}</h3>${historical.corrections.map((item,i)=>`<article class="nvm-correction" id="comparison-correction-${i+1}"><div class="nvm-correction-question"><p class="nvm-correction-label"><span>${String(i+1).padStart(2,'0')}</span>${isEnglish?'Claim to Correct':'需校正的說法'}</p><h4>${esc(item.old)}</h4></div><div class="nvm-correction-answer"><h5>${isEnglish?'Updated Interpretation':'更新判讀'}</h5>${paras(item.current)}<div class="nvm-correction-reason"><h5>${isEnglish?'Why It Matters':'差異原因'}</h5>${paras(item.reason)}</div>${cite(item.sourceIds)}<a class="nvm-benchmark-return" href="#comparison-corrections">${isEnglish?'Back to the Corrections':'回到差異校正'} ↑</a></div></article>`).join('')}</section></section>`;
}
...
function comparisonPanel() {
  ...
  <p class="nvm-comparison-history-link"><a href="#comparison-history">${isEnglish?'Historical Course Table and Corrections':'歷史課程表與差異校正'} <span aria-hidden="true">↓</span></a></p>
  ...
  ${historicalComparison(historical)}</article>`;
}
...
{id:'comparison',zh:'技術比較與整合條件',en:'Technology Comparisons and Integration Conditions',descZh:'在相同條件下閱讀數值、歷史表與具名實作案例。',descEn:'Read figures, historical comparisons and named implementations within their stated conditions.'},
...
const sourcePanel = `<article id="sources" class="nvm-panel nvm-sources-editorial" data-nvm-panel>${panelHeader('可追溯來源與共用資料','每一個結論，都能回到它的證據','這個網站以結構化專題資料為共同來源，包含位元單元、操作、天花板、專利、比較條件與年度紀錄。後續簡報可由同一批資料取材，保留來源與限制。')}<div class="nvm-note">${bullets(intro.evidenceRules)}</div><div class="nvm-actions"><a href="data/NVM知識資料.json" download>下載完整結構化資料</a><a href="data/NVM技術專題.md" download>下載完整專題文字</a></div><p class="nvm-small">研究版本：${intro.revision}。資料檔的分類、來源、成熟度與限制保留獨立欄位；正式簡報的版面與取材可另外編排。</p><div class="nvm-filters nvm-source-filters"><label>搜尋來源<input id="nvm-source-search" type="search" placeholder="公司、技術、專利、年份或來源編號"></label><button type="button" id="nvm-source-reset">${isEnglish?'Clear Search':'清除搜尋'}</button></div><p id="nvm-source-count" role="status" class="nvm-results">${sources.length} 筆來源紀錄</p><p id="nvm-source-empty" class="nvm-source-empty" role="status" hidden>${isEnglish?'No sources match. Clear the search or use a company, title or source ID.':'沒有符合的來源。請清除搜尋，或改用公司、文獻標題與來源編號。'}</p>${sources.map(source=>`<details class="nvm-disclosure" id="source-${esc(source.id)}" data-source-record><summary><span class="nvm-source-id">${esc(source.id)}</span><span class="nvm-source-title">${esc(source.label)}</span><span class="nvm-source-meta">${esc(source.kind)} · ${esc(sourceDate(source))}</span></summary><div><dl><dt>來源類別</dt><dd>${esc(source.kind)}</dd><dt>日期／查閱基準</dt><dd>${esc(sourceDate(source))}</dd><dt>定位</dt><dd>${esc(source.locator)}</dd><dt>支持範圍與限制</dt><dd>${esc(source.limit)}</dd><dt>原始連結</dt><dd><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">開啟原始來源</a></dd></dl></div></details>`).join('')}</article>`;
const patentPanel = `<article id="patents" class="nvm-panel nvm-patents-editorial" data-nvm-panel>${panelHeader('代表專利導讀',`${patents.length} 件專利，回到具體設計問題`,'每件先看它要解決的問題，再讀結構、操作與權利項。這裡是種子專利研究索引，未完成全家族法律狀態與自由實施分析。')}<aside class="nvm-note">優先權日、公開日與核准日不同。專利中的偏壓、材料、圖例與尺寸只屬於該實施例；專利受讓公司與其量產產品採用的實作，須另有證據連結。</aside><div class="nvm-reference-controls nvm-patent-controls"><label>${isEnglish?'Search Patents':'搜尋專利'}<input id="nvm-patent-search" type="search" placeholder="${isEnglish?'Patent ID, design problem or assignee':'專利編號、設計問題或受讓機構'}"></label><label>${isEnglish?'Technology':'技術專題'}<select id="nvm-patent-topic"><option value="">${isEnglish?'All Technologies':'全部技術'}</option>${topics.filter(topic=>topic.patents.length).map(topic=>`<option value="${esc(topic.id)}">${esc(topic.title)}</option>`).join('')}</select></label><button id="nvm-patent-reset" type="button">${isEnglish?'Clear Filters':'清除篩選'}</button></div><p class="nvm-reference-count" id="nvm-patent-count" role="status">${patents.length} ${isEnglish?'patent studies':'件研究專利'}</p><p class="nvm-reference-empty" id="nvm-patent-empty" role="status" hidden>${isEnglish?'No matching patents. Clear the filters or try a shorter keyword.':'沒有符合的專利。請清除篩選，或縮短關鍵字。'}</p>${topics.filter(topic=>topic.patents.length).map(topic=>`<section class="nvm-patent-group" data-patent-group="${esc(topic.id)}"><h3>${esc(topic.title)}</h3>${topic.patents.map(patent=>patentDetail(patent,topic)).join('')}</section>`).join('')}</article>`;
const glossaryPanel = `<article id="glossary" class="nvm-panel nvm-glossary-editorial" data-nvm-panel>${panelHeader('共同語言','讀懂跨技術比較需要的詞彙','先辨別數字的物理意義與測量層級，才能比較其成本與適用範圍。')}<div class="nvm-reference-controls nvm-glossary-controls"><label>${isEnglish?'Search Terms and Definitions':'搜尋術語與定義'}<input id="nvm-glossary-search" type="search" placeholder="${isEnglish?'Term, abbreviation or concept':'術語、縮寫或概念'}"></label><button id="nvm-glossary-reset" type="button">${isEnglish?'Clear Search':'清除搜尋'}</button></div><p class="nvm-reference-count" id="nvm-glossary-count" role="status">${comparison.glossary.length} ${isEnglish?'terms':'個詞彙'}</p><p class="nvm-reference-empty" id="nvm-glossary-empty" role="status" hidden>${isEnglish?'No matching terms. Clear the search or try another keyword.':'沒有符合的詞彙。請清除搜尋，或更換關鍵字。'}</p><dl class="nvm-glossary">${comparison.glossary.map((item,i)=>`<div id="glossary-term-${String(i+1).padStart(2,'0')}" data-glossary-record data-reference-search="${esc(item.term+' '+item.definition)}"><dt><span class="nvm-term-number" aria-hidden="true">${String(i+1).padStart(2,'0')}</span><a href="#glossary-term-${String(i+1).padStart(2,'0')}">${esc(item.term)}</a></dt><dd>${esc(item.definition)}</dd></div>`).join('')}</dl></article>`;

let html = `<!doctype html>
<html lang="${isEnglish ? 'en' : 'zh-Hant'}" data-language="${language}" data-content-language="${language}" data-language-en="NVM技術全景.html" data-language-zh="NVM技術全景中文.html"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(isEnglish ? 'An NVM reference library covering named IP cells, technology lineage, foundry integration, device physics, patents and operating diagrams.' : 'NVM 專題資料庫：具名 IP 單元、技術沿革、晶圓代工整合、儲存物理、專利與操作圖解。')}"><meta name="theme-color" content="#0a1118"><title>${esc(intro.title)} · NVM Knowledge Hub</title><link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="NVM技術全景.css?v=20260910-library"><link rel="stylesheet" href="NVM專業元件圖.css?v=20260910"><link rel="stylesheet" href="NVM工程圖解.css?v=20260911-editorial10"><link rel="stylesheet" href="NVMIP單元.css?v=20260910-lineage"><link rel="stylesheet" href="NVM產研比較.css?v=20260910-editorial6"><link rel="stylesheet" href="商用閱讀介面.css?v=20260911-editorial9"><script src="site-language.js?v=20260910-bilingual"></script></head>
...
<header class="nvm-header"><a class="brand" href="index.html" aria-label="NVM 知識中心首頁"><strong>NVM</strong><span>知識中心<br>物理與技術全景</span></a><nav aria-label="知識中心導覽"><a href="index.html#layer-foundations">${isEnglish?'Technology Foundations':'技術基礎'}</a><a href="index.html#layer-ip-process">${isEnglish?'IP and Processes':'IP 與製程'}</a><a href="index.html#layer-applications">${isEnglish?'Applications and Systems':'應用與系統'}</a><a href="index.html#layer-resources">${isEnglish?'Literature and Tools':'文獻與工具'}</a><button type="button" class="language-toggle" aria-label="${isEnglish ? 'Switch to Traditional Chinese' : '切換為英文'}"><span lang="en">EN</span><i aria-hidden="true">/</i><span lang="zh-Hant">中文</span></button></nav></header>
...
<footer class="nvm-footer"><p>NVM 知識中心 · 公開來源研究 · ${intro.revision}</p><a href="index.html">返回知識中心</a> · <a href="#sources">查看來源與共用資料</a></footer><script type="module" src="NVM技術全景.js?v=20260911-editorial9"></script><script type="module" src="NVM工程圖解.js?v=20260911-editorial10"></script></body></html>\n`;
// 全頁標題使用標籤語法，不保留句尾標點。
html = html.replace(/<h([1-6])\b[^>]*>[\s\S]*?<\/h\1>/giu, heading => heading.replace(/[。.!?！？](?=(?:[”’"'）)}\]】》]+)?\s*(?:<br\b[^>]*>|<\/(?:span|em|h[1-6])>))/giu,''));
const packageData = { schemaVersion: '1.4', revision: intro.revision, language: isEnglish ? 'en' : 'zh-Hant', classification: 'Public', title: intro.title, intro, topics, comparison, foundry, research, sources, engineering, ipCurriculum, provenance: { sourceFiles: ['NVM全景導論.json','NVM電荷專題.json','NVM新興專題.json','NVM比較與系統.json','NVM晶圓代工路線圖.json','NVMIP單元導論.json','NVM產研比較.json'].map(name => name.replace('.json', `${dataSuffix}.json`)), engineeringSources: ['NVM專利附圖.json', '../scripts/NVM電荷操作圖.mjs', '../scripts/NVM新興操作圖.mjs', '../scripts/NVM專利圖解.mjs','../scripts/NVM力旺IP圖解.mjs','../scripts/NVM新興IP圖解.mjs','../scripts/NVMYMCIP圖解.mjs','../scripts/NVMSynopsysOTP圖解.mjs','../scripts/NVMImpinjIP圖解.mjs'], reviewScope: isEnglish ? 'Public-source research and instructional review; shared source for future presentations' : '公開來源與教學內容查核；網頁為後續簡報的取材來源' } };
...
const chineseResearch = isEnglish ? JSON.parse(fs.readFileSync(path.join(root,'data/NVM產研比較.json'),'utf8')) : research;
...
const chineseIPIndex = isEnglish ? JSON.parse(fs.readFileSync(path.join(root,'data/NVMIP單元導論.json'),'utf8')) : ipIndex;
const searchEntries = [
 ...
 {title_zh:'IP 單元與操作原理',title_en:'IP Cells and Operating Principles',url:'NVM技術全景.html#ip-directory',tags:'IP 單元 目錄 operating principles directory'},
 {title_zh:'IP 技術沿革與產品承接',title_en:'IP Technology Lineage and Product Succession',url:'NVM技術全景.html#ip-lineage',tags:'Synopsys 收購 承接 技術沿革 acquisition lineage Kilopass Sidense Impinj Virage Logic AEON'},
 {title_zh:'儲存物理專題',title_en:'Device Physics Topics',url:'NVM技術全景.html#nvm-physics-overview',tags:'物理 基礎 儲存 standalone physics background'},
 ...ipCurriculum.units.map(unit=>({title_zh:chineseIPIndex.units.find(item=>item.id===unit.id).title,title_en:unit.title,url:`NVM技術全景.html#ip-${unit.id}`,tags:[unit.vendor,unit.shortTitle,unit.summary,unit.program,unit.reverse,unit.readout,chineseIPIndex.units.find(item=>item.id===unit.id).summary].join(' ')})),
 ...topics.map(topic=>({title_zh:chineseTopics.find(t=>t.id===topic.id).title,title_en:topic.title,url:`NVM技術全景.html#topic-${topic.id}`,tags:[topic.storage,topic.summary,topic.maturity.claim,chineseTopics.find(t=>t.id===topic.id).storage,chineseTopics.find(t=>t.id===topic.id).summary,...(topic.integrationRoutes||[]).flatMap(route=>[route.title,route.provider,route.polyLayers,route.controlTopology]),...topic.sourceIds.map(id=>sourceMap.get(id)?.label||''),...topic.patents.map(p=>p.id)].join(' ')})),
 ...
 ...research.landscape.map(r=>({title_zh:chineseResearch.landscape.find(c=>c.id===r.id).name+' · '+r.technology,title_en:r.name+' · '+r.technology,url:'NVM技術全景.html#company-'+r.id,tags:[r.name,r.technology,r.claim,r.search,chineseResearch.landscape.find(c=>c.id===r.id).claim].join(' ')})),
 ...research.profiles.map(p=>({title_zh:chineseResearch.profiles.find(c=>c.id===p.id).title,title_en:p.title,url:'NVM技術全景.html#research-'+p.id,tags:p.name+' '+p.summary})),
 {title_zh:'全球 NVM 產業與研究地圖',title_en:'Global NVM Industry and Research Map',url:'NVM技術全景.html#ecosystem',tags:'Everspin Samsung Intel MRAM ReRAM PCM FeRAM FeFET 產業 研究 工研院'},
 {title_zh:'GF／TSMC／UMC 年度路線圖',title_en:'GF / TSMC / UMC Roadmap',url:'NVM技術全景.html#foundry',tags:'GLOBALFOUNDRIES TSMC eMRAM ReRAM RRAM eNVM roadmap 22FDX 12LP AutoPro150'},
