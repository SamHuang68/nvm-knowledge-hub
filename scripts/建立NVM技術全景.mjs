...
const originalSources = [...intro.sources, ...charge.sources, ...emerging.sources, ...comparison.sources, ...foundry.sources];
...
const serialized = JSON.stringify({ intro, topics, sources, comparison, foundry, engineering });
...
<html lang="${isEnglish ? 'en' : 'zh-Hant'}" data-language="${language}" data-content-language="${language}" data-language-en="NVM技術全景.html" data-language-zh="NVM技術全景中文.html"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(intro.subtitle)}：十六個 NVM 技術專題、位元單元操作、比較表、專利與 GLOBALFOUNDRIES／TSMC MRAM、ReRAM 年度路線圖。"><meta name="theme-color" content="#0a1118"><title>${esc(intro.title)} · NVM Knowledge Hub</title><link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="NVM技術全景.css?v=20260910-bilingual"><link rel="stylesheet" href="NVM專業元件圖.css?v=20260910"><link rel="stylesheet" href="NVM工程圖解.css?v=20260910"><script src="site-language.js?v=20260910-bilingual"></script></head>
...
<section class="nvm-hero"><div><p class="nvm-kicker">NVM 技術全景 · 研究版本 ${intro.revision}</p><h1>從位元單元<br><em>看懂資料如何留下來</em></h1><p>十六個技術專題，從儲存物理、操作與專利，走向可量產的陣列與系統。每項優勢，都連著一項必須理解的代價。</p></div><div class="nvm-hero-figure"><div class="nvm-figure-label">${isEnglish ? 'DEVICE PHYSICS / 16 TECHNOLOGY & IP STUDIES' : '元件物理／十六個技術專題'}</div>${bitcellFigure('stt','STT-MRAM',language,true)}<div class="nvm-figure-caption">${isEnglish ? 'Perpendicular MTJ · Free Layer / Barrier / Reference Layer' : '垂直磁化接面 · 自由層／障壁／參考層'}</div></div></section>
...
<footer class="nvm-footer"><p>NVM 知識中心 · 公開來源研究 · ${intro.revision}</p><a href="index.html">返回知識中心</a> · <a href="#sources">查看來源與共用資料</a></footer><script type="module" src="NVM技術全景.js?v=20260910-operations"></script><script type="module" src="NVM工程圖解.js?v=20260910"></script></body></html>\n`;
// 全頁標題使用標籤語法，不保留句尾標點。
html = html.replace(/<h([1-6])\b[^>]*>[\s\S]*?<\/h\1>/giu, heading => heading.replace(/[。.!?！？](?=(?:[”’"'）)}\]】》]+)?\s*(?:<br\b[^>]*>|<\/(?:span|em|h[1-6])>))/giu,''));
const packageData = { schemaVersion: '1.1', revision: intro.revision, language: isEnglish ? 'en' : 'zh-Hant', classification: 'Public', title: intro.title, intro, topics, comparison, foundry, sources, engineering, provenance: { sourceFiles: ['NVM全景導論.json','NVM電荷專題.json','NVM新興專題.json','NVM比較與系統.json','NVM晶圓代工路線圖.json'].map(name => name.replace('.json', `${dataSuffix}.json`)), engineeringSources: ['NVM專利附圖.json', '../scripts/NVM電荷操作圖.mjs', '../scripts/NVM新興操作圖.mjs', '../scripts/NVM專利圖解.mjs'], reviewScope: isEnglish ? 'Public-source research and instructional review; shared source for future presentations' : '公開來源與教學內容查核；網頁為後續簡報的取材來源' } };
