...
export function renderIPDirectory(curriculum, language) {
  ...
  return `<article class="nvm-panel nvm-ip-directory" id="ip-directory" data-nvm-panel><header><h2>${t('IP 單元與操作原理','IP Cells and Operating Principles')}</h2><p>${t('先選一款 IP，沿同一單元看寫入、反向更新與讀取。OTP、MTP、MRAM、ReRAM 的差異直接落在圖中的儲存區與操作路徑。','Choose an IP and follow the same cell through programming, reverse update and read. The storage region and operating paths explain the differences among OTP, MTP, MRAM and ReRAM.')}</p></header>${curriculum.groups.map(group => `<section class="nvm-ip-group"><div class="nvm-ip-group-label"><span>${esc(group.label)}</span><p>${esc(group.description)}</p></div><div class="nvm-ip-rows">${curriculum.units.filter(unit => unit.group === group.id).map(unit => `<article class="nvm-ip-row" data-ip-entry="${esc(unit.id)}"><div><p class="nvm-ip-vendor">${esc(unit.vendor)}</p><h4><a href="#ip-${esc(unit.id)}">${esc(unit.shortTitle)}</a></h4><span class="nvm-ip-entry-link">${t('開啟單元與操作圖','Explore the Cell and Operations')} →</span></div><dl><div><dt>${t('寫入','Program')}</dt><dd>${unit.group === 'mtp' ? 'PGM · ' : ''}${esc(unit.program)}</dd></div><div><dt>${t('反向操作','Reverse Operation')}</dt><dd>${unit.group === 'mtp' ? 'ERS · ' : ''}${esc(unit.reverse)}</dd></div><div><dt>${t('讀取','Read')}</dt><dd>${esc(unit.readout)}</dd></div></dl></article>`).join('')}</div></section>`).join('')}</article>`;
}
