...
export function renderIPLineage(curriculum,language,cite){
  ...
  if(!lineage)return '';
  const eventCount=lineage.entries.reduce((sum,entry)=>sum+entry.events.length,0);
  return `<article id="ip-lineage" class="nvm-panel nvm-ip-lineage nvm-lineage-editorial" data-nvm-panel><header class="nvm-lineage-cover"><div><p class="nvm-kicker">${t('IP 與製程 · 技術沿革','IP AND PROCESSES · TECHNOLOGY LINEAGE')}</p><h2>${esc(lineage.title)}</h2><div class="nvm-lede">${paragraphs(lineage.intro)}</div></div><dl><div><dt>${t('技術家族','Technology Families')}</dt><dd>${lineage.entries.length}</dd></div><div><dt>${t('公開事件','Public Events')}</dt><dd>${eventCount}</dd></div></dl></header>
  <nav id="lineage-contents" class="nvm-lineage-index" aria-label="${t('技術家族','Technology Families')}">${lineage.entries.map((entry,i)=>`<a href="#lineage-${esc(entry.id)}"><span>${String(i+1).padStart(2,'0')}</span><strong>${esc(entry.title)}</strong><small>${entry.events.length} ${t('個公開事件','Public Events')}</small></a>`).join('')}</nav>
  ${lineage.entries.map((entry,i)=>`<section id="lineage-${esc(entry.id)}" class="nvm-lineage-entry"><header class="nvm-lineage-family-heading"><span aria-hidden="true">${String(i+1).padStart(2,'0')}</span><div><h3>${esc(entry.title)}</h3><div class="nvm-lineage-summary">${paragraphs(entry.summary)}</div></div></header><nav class="nvm-lineage-section-links" aria-label="${esc(entry.title)} ${t('閱讀章節','Reading Sections')}"><a href="#lineage-${esc(entry.id)}-events">${t('公開事件','Public Events')}</a><a href="#lineage-${esc(entry.id)}-context">${t('後續產品','Subsequent Products')}</a><a href="#lineage-${esc(entry.id)}-boundary">${t('圖解邊界','Cell-Study Boundary')}</a></nav>
  <section class="nvm-lineage-events-block" id="lineage-${esc(entry.id)}-events"><h4>${t('公開事件與原始來源','Public Events and Original Sources')}</h4><ol class="nvm-lineage-events">${entry.events.map((event,j)=>`<li class="nvm-lineage-event" id="lineage-${esc(entry.id)}-event-${j+1}"><a class="nvm-lineage-date" href="#lineage-${esc(entry.id)}-event-${j+1}" aria-label="${esc(event.date+' · '+event.title)}"><time datetime="${esc(event.date)}">${esc(event.date)}</time></a><div class="nvm-lineage-event-body"><h5>${esc(event.title)}</h5>${paragraphs(event.body)}${cite(event.sourceIds)}</div></li>`).join('')}</ol></section>
  <div class="nvm-lineage-context"><section id="lineage-${esc(entry.id)}-context" class="nvm-lineage-current"><h4>${t('後續產品與現況','Subsequent Products and Current Context')}</h4>${paragraphs(entry.currentContext)}</section><section id="lineage-${esc(entry.id)}-boundary" class="nvm-lineage-boundary"><h4>${t('與單元圖解的對應','Relation to the Cell Study')}</h4>${paragraphs(entry.cellBoundary)}</section></div><div class="nvm-lineage-context-sources">${cite(entry.sourceIds)}</div><nav class="nvm-lineage-family-actions" aria-label="${t('沿革接續','Continue the Lineage Study')}"><a class="nvm-lineage-cell-link" href="#ip-${esc(entry.unitId)}">${t('閱讀這個家族的單元與操作','Explore This Family’s Cell and Operations')} →</a><a href="#lineage-contents">${t('回到家族索引','Back to the Family Index')} ↑</a></nav></section>`).join('')}</article>`;
