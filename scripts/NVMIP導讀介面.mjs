...
export function renderIPDirectory(curriculum, language) {
  ...
  <header class="nvm-ip-directory-cover"><div><p class="nvm-kicker">${t('IP 與製程 · 單元導讀','IP AND PROCESSES · CELL STUDIES')}</p><h2>${t('IP 單元與操作原理','IP Cells and Operating Principles')}</h2><p class="nvm-lede">${t('先選一款 IP，沿同一單元看寫入、反向更新與讀取。OTP、MTP、eFlash、MRAM、ReRAM 的差異直接落在圖中的儲存區與操作路徑。','Choose an IP and follow the same cell through programming, reverse update and read. The storage region and operating paths explain the differences among OTP, MTP, eFlash, MRAM and ReRAM.')}</p></div><dl><div><dt>${t('具名單元','Named Cells')}</dt><dd>${curriculum.units.length}</dd></div><div><dt>${t('技術家族','Technology Families')}</dt><dd>${curriculum.groups.length}</dd></div></dl></header>
  ...
  ${curriculum.groups.map(group => `<section class="nvm-ip-group" id="ip-group-${esc(group.id)}"><div class="nvm-ip-group-label"><h3>${esc(group.label)}</h3><p>${esc(group.description)}</p></div><div class="nvm-ip-rows">${curriculum.units.filter(unit => unit.group === group.id).map(unit => `<article class="nvm-ip-row" data-ip-entry="${esc(unit.id)}"><div><p class="nvm-ip-vendor">${esc(unit.vendor)}</p><h4><a href="#ip-${esc(unit.id)}">${esc(unit.shortTitle)}</a></h4><a class="nvm-ip-entry-link" href="#ip-${esc(unit.id)}">${t('開啟單元與操作圖','Explore the Cell and Operations')} <span aria-hidden="true">↗</span></a></div><dl><div><dt>${t('寫入','Program')}</dt><dd>${['mtp','eflash'].includes(unit.group) ? 'PGM · ' : ''}${esc(unit.program)}</dd></div><div><dt>${t('反向操作','Reverse Operation')}</dt><dd>${['mtp','eflash'].includes(unit.group) ? 'ERS · ' : ''}${esc(unit.reverse)}</dd></div><div><dt>${t('讀取','Read')}</dt><dd>${esc(unit.readout)}</dd></div></dl></article>`).join('')}</div></section>`).join('')}</article>`;
}
...
<nav class="nvm-bottom-nav" aria-label="${t(esc(unit.shortTitle)+' IP 單元接續','Continue Through IP Cells: '+esc(unit.shortTitle))}"><a href="${index ? '#ip-'+esc(curriculum.units[index-1].id) : '#ip-directory'}">${index ? t('上一款：','Previous: ')+esc(curriculum.units[index-1].shortTitle) : t('回到 IP 目錄','Back to the IP Directory')}</a><a href="${index < curriculum.units.length-1 ? '#ip-'+esc(curriculum.units[index+1].id) : '#ip-directory'}">${index < curriculum.units.length-1 ? t('下一款：','Next: ')+esc(curriculum.units[index+1].shortTitle) : t('回到 IP 目錄','Back to the IP Directory')}</a></nav></article>`).join('');
}
